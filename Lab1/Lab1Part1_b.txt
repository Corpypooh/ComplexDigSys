...
architecture fsm of Lab1Part1_b is
	component flipflop
	port (
...
begin
	...
	--State A, 0
	D_in(0) <= '0';
	S0: flipflop2 PORT MAP (D_in(0), clk, rst, NOT rst, Q_out(0));
	
	--State B, 1
	D_in(1) <= (NOT Q_out(0) OR Q_out(5) OR Q_out(6) OR Q_out(7) OR Q_out(8)) AND NOT (w);
	...
	--State F, 5
	D_in(5) <= (NOT Q_out(0) OR Q_out(1) OR Q_out(2) OR Q_out(3) OR Q_out(4)) AND (w);
